...
失败语义：
- 模型异常或超出 RunLimits → status=FAILED，error 结构化记录；
...
class AgentRuntime:
    ...
    async def run(
        ...
        *,
        ...
        （为第二阶段的暂停/恢复预留）。tool_names 限定本次运行可见的工具
        （技能子运行用它排除其他技能，防止递归）。
        """
        ...
        try:
            while True:
                if state.step >= self.limits.max_steps:
                    return self._fail(
                        state, f"max_steps_exceeded: limit is {self.limits.max_steps}"
                    )
                state.step += 1
                ...
                try:
                    ...
                    return self._fail(
                        state, f"model_error: {type(exc).__name__}: {exc}"
                    )
            ...
            return self._fail(state, f"runtime_error: {type(exc).__name__}: {exc}")
    ...
    def _fail(self, state: RunState, error: str) -> RunState:
        ...
        state.error = error
        state.updated_at = time.time()
        self._emit(state, RunEventType.RUN_FAILED, error=error)
        return state
